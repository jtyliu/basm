...
opcodes = [
    ...
    Opcode('i32.load16_u', 0x2f, MemoryImm, '($base: iPTR) : (i32)', IF.M | IF.U),
    Opcode('i64.load8_s', 0x30, MemoryImm, '($base: iPTR) : (i64)', IF.M | IF.S),
    Opcode('i64.load8_u', 0x31, MemoryImm, '($base: iPTR) : (i64)', IF.M | IF.U),
    Opcode('i64.load16_s', 0x32, MemoryImm, '($base: iPTR) : (i64)', IF.M | IF.S),
    ...
    Opcode('i64.store16', 0x3d, MemoryImm, '($base: iPTR, $value: i64) : ()', IF.M | IF.G),
    ...
    Opcode('i32.ne', 0x47, None, '(i32, i32) : (i32)', IF.C | IF.G),
    Opcode('i32.lt_s', 0x48, None, '(i32, i32) : (i32)', IF.C | IF.S),
    Opcode('i32.lt_u', 0x49, None, '(i32, i32) : (i32)', IF.C | IF.U),
    Opcode('i32.gt_s', 0x4a, None, '(i32, i32) : (i32)', IF.C | IF.S),
    Opcode('i32.gt_u', 0x4b, None, '(i32, i32) : (i32)', IF.C | IF.U),
    Opcode('i32.le_s', 0x4c, None, '(i32, i32) : (i32)', IF.C | IF.S),
    Opcode('i32.le_u', 0x4d, None, '(i32, i32) : (i32)', IF.C | IF.U),
    Opcode('i32.ge_s', 0x4e, None, '(i32, i32) : (i32)', IF.C | IF.S),
    Opcode('i32.ge_u', 0x4f, None, '(i32, i32) : (i32)', IF.C | IF.U),
    Opcode('i64.eqz', 0x50, None, '(i64) : (i32)', IF.G),
    Opcode('i64.eq', 0x51, None, '(i64, i64) : (i32)', IF.C | IF.G),
    Opcode('i64.ne', 0x52, None, '(i64, i64) : (i32)', IF.C | IF.G),
    Opcode('i64.lt_s', 0x53, None, '(i64, i64) : (i32)', IF.C | IF.S),
    Opcode('i64.lt_u', 0x54, None, '(i64, i64) : (i32)', IF.C | IF.U),
    Opcode('i64.gt_s', 0x55, None, '(i64, i64) : (i32)', IF.C | IF.S),
    Opcode('i64.gt_u', 0x56, None, '(i64, i64) : (i32)', IF.C | IF.U),
    Opcode('i64.le_s', 0x57, None, '(i64, i64) : (i32)', IF.C | IF.S),
    Opcode('i64.le_u', 0x58, None, '(i64, i64) : (i32)', IF.C | IF.U),
    Opcode('i64.ge_s', 0x59, None, '(i64, i64) : (i32)', IF.C | IF.S),
    Opcode('i64.ge_u', 0x5a, None, '(i64, i64) : (i32)', IF.C | IF.U),
    Opcode('f32.eq', 0x5b, None, '(f32, f32) : (i32)', IF.C | IF.F),
    Opcode('f32.ne', 0x5c, None, '(f32, f32) : (i32)', IF.C | IF.F),
    Opcode('f32.lt', 0x5d, None, '(f32, f32) : (i32)', IF.C | IF.F),
    Opcode('f32.gt', 0x5e, None, '(f32, f32) : (i32)', IF.C | IF.F),
    Opcode('f32.le', 0x5f, None, '(f32, f32) : (i32)', IF.C | IF.F),
    Opcode('f32.ge', 0x60, None, '(f32, f32) : (i32)', IF.C | IF.F),
    Opcode('f64.eq', 0x61, None, '(f64, f64) : (i32)', IF.C | IF.F),
    Opcode('f64.ne', 0x62, None, '(f64, f64) : (i32)', IF.C | IF.F),
    Opcode('f64.lt', 0x63, None, '(f64, f64) : (i32)', IF.C | IF.F),
    Opcode('f64.gt', 0x64, None, '(f64, f64) : (i32)', IF.C | IF.F),
    Opcode('f64.le', 0x65, None, '(f64, f64) : (i32)', IF.C | IF.F),
    Opcode('f64.ge', 0x66, None, '(f64, f64) : (i32)', IF.C | IF.F),
    ...
    Opcode('f32.ceil', 0x8d, None, '(f32) : (f32)', IF.F),
    Opcode('f32.floor', 0x8e, None, '(f32) : (f32)', IF.F),
    Opcode('f32.trunc', 0x8f, None, '(f32) : (f32)', IF.F),
    Opcode('f32.nearest', 0x90, None, '(f32) : (f32)', IF.F),
    Opcode('f32.sqrt', 0x91, None, '(f32) : (f32)', IF.F),
    Opcode('f32.add', 0x92, None, '(f32, f32) : (f32)', IF.F),
    Opcode('f32.sub', 0x93, None, '(f32, f32) : (f32)', IF.F),
    Opcode('f32.mul', 0x94, None, '(f32, f32) : (f32)', IF.F),
    Opcode('f32.div', 0x95, None, '(f32, f32) : (f32)', IF.F),
    ...
    Opcode('f64.abs', 0x99, None, '(f64) : (f64)', IF.E),
    Opcode('f64.neg', 0x9a, None, '(f64) : (f64)', IF.E),
    Opcode('f64.ceil', 0x9b, None, '(f64) : (f64)', IF.F),
    Opcode('f64.floor', 0x9c, None, '(f64) : (f64)', IF.F),
    Opcode('f64.trunc', 0x9d, None, '(f64) : (f64)', IF.F),
    Opcode('f64.nearest', 0x9e, None, '(f64) : (f64)', IF.F),
    Opcode('f64.sqrt', 0x9f, None, '(f64) : (f64)', IF.F),
    Opcode('f64.add', 0xa0, None, '(f64, f64) : (f64)', IF.F),
    Opcode('f64.sub', 0xa1, None, '(f64, f64) : (f64)', IF.F),
    Opcode('f64.mul', 0xa2, None, '(f64, f64) : (f64)', IF.F),
    Opcode('f64.div', 0xa3, None, '(f64, f64) : (f64)', IF.F),
    ...
    Opcode('i64.extend_i32_u', 0xad, None, '(i32) : (i64)', IF.U),
